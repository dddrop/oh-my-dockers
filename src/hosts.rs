...
pub fn add_project_domains(project_name: &str, domains: &[String]) -> Result<()> {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        // If we're in the section, check if we've hit the next section or non-domain line
            ...
            else if !trimmed.is_empty() && !trimmed.starts_with("#") && !trimmed.starts_with("127.0.0.1") {
    ...
    if let (Some(start), Some(end)) = (project_start_idx, project_end_idx) {
        println!();
        println!("{} Found existing entries for project {}:", "ℹ".blue(), project_name.bright_white());
        println!();
    ...
    if project_start_idx.is_some() || project_end_idx.is_some() {
        println!("{}", "Will replace the existing entries with:".bright_white());
    } else {
    ...
    if let Err(e) = fs::write(hosts_path, &new_content) {
        ...
        println!(
            "{} Attempting to write with sudo privileges...",
            "ℹ".blue()
        );
...
pub fn remove_project_domains(project_name: &str) -> Result<()> {
    ...
    if let (Some(start), Some(end)) = (project_start_idx, project_end_idx) {
        ...
        println!("{} No entries found for project {}", "ℹ".blue(), project_name.bright_white());
        return Ok(());
    }
    ...
    if let Err(e) = fs::write(hosts_path, &new_content) {
        ...
        if !output.status.success() {
            let error_msg = String::from_utf8_lossy(&output.stderr);
            anyhow::bail!(
                "Failed to write /etc/hosts with sudo: {}. Error: {}. Please run with sudo.",
                e,
                error_msg
            );
        }
...
pub fn list_managed_domains() -> Result<()> {
    ...
    let mut project_domains: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
